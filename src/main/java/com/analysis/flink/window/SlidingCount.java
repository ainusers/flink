...
public class SlidingCount {
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 时间滚动窗口
        DataStream<Tuple2<String, Double>> slidingCountStream = flatMapStream.map(new MapFunction<SensorReading, Tuple2<String, Double>>() {
            @Override
            public Tuple2<String, Double> map(SensorReading sensorReading) throws Exception {
                ...
                .keyBy(data -> data.f0)
                // 每收到2个相同 key 的数据就计算一次， 每一次计算的 window 范围是 10 个元素
                .countWindow(10, 2)
                .maxBy(1);

        // 打印读取数据
        slidingCountStream.print();
