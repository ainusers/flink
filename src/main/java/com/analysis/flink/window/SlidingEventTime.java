...
public class SlidingEventTime {
    public static void main(String[] args) throws Exception{
        ...
        // 时间滚动窗口
        DataStream<Tuple2<String, Double>> slideEventTimeStream = flatMapStream.map(new MapFunction<SensorReading, Tuple2<String, Double>>() {
            @Override
            public Tuple2<String, Double> map(SensorReading sensorReading) throws Exception {
                ...
                .keyBy(data -> data.f0)
                // 每 5s 就计算输出结果一次， 每一次计算的 window 范围是 15s 内的所有元素
                .timeWindow(Time.seconds(15), Time.seconds(5))
                .minBy(1);

        // 打印读取数据
        slideEventTimeStream.print();
